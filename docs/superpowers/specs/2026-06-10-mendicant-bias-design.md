...
- **Migration tasks (infra, outside this repo):** provision OS + runner (new label) + Caddy + SurrealDB; migrate prod data with a restore-verified backup (per the established `/data/prod_backups/` discipline); deploy-workflow retarget (the fenrir migration provides the playbook in reverse); llama.cpp service for Hypebot; demote fenrir to fallback/xray duty; update CLAUDE.md (Deploy section, gotchas) and project memory when cut over.
