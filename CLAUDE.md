...
- **Deploy:** push to `main` auto-deploys to production (**fenrir**, a Raspberry Pi 4B / aarch64 — migrated off the x86_64 `novahome` 2026-06-04, now decommissioned) via a self-hosted GitHub Actions runner labelled `fenrir` (`.github/workflows/deploy.yml`): hard-resets `/home/damien/authlyn-interactive` to the pushed commit → **native release build on the Pi (slow; cold build ~90 min, `timeout-minutes: 120`)** → gzip backup of prod DB to `/data/prod_backups/` → `sudo /opt/authlyn/deploy.sh` (swap + health-check + auto-rollback). Docs-only pushes (`**/*.md` — incl. root `CLAUDE.md` — and `docs/**`) are excluded (verified: a CLAUDE.md-only push creates no deploy run; a *mixed* CLAUDE.md+code push still deploys, by design). Manual deploy of an arbitrary working tree is the `deploy` skill. NOTE: the runner shares that repo dir and hard-resets it every deploy — never leave uncommitted work in `/home/damien/authlyn-interactive`.
...
- Building `ssr` needs system libcurl (web-push) and the `image` crate at build time — native, ssr-only. On the aarch64 prod host (fenrir) the release build additionally needs `cmake` (aws-lc-sys C build) and a working `wasm-opt` — see the Pi wasm-opt gotcha below.
- **Pi `wasm-opt` SIGILL (fenrir, Cortex-A72 / ARMv8.0):** cargo-leptos downloads a prebuilt `wasm-opt` (binaryen `version_123`) whose official aarch64 binary uses ARMv8.1+ instructions the Pi 4B's A72 lacks → it crashes with `Illegal instruction` (exit 132) and `cargo leptos build --release` dies at `wasm-opt optimization failed` (server bin + `site/` are already built; only the wasm-opt tail fails). Fix in place: Debian's `binaryen` (apt, built for the ARMv8.0 baseline) is copied over the cached binary at `~/.cache/cargo-leptos/wasm-opt-version_123/.../bin/wasm-opt` (persists across deploys, which only reset the repo). If the cache is wiped or cargo-leptos bumps the binaryen version, re-apply: `sudo apt install -y binaryen && cp /usr/bin/wasm-opt <that cached path>`. fenrir also runs a 6 GB low-priority `/swapfile` (it ships with only ~2 GB zram) to backstop build memory.
...
- Mobile PWA safe-area: the installed app reserves notch / Dynamic Island / home-indicator space via `viewport-fit=cover` (`src/app.rs` viewport meta) + `env(safe-area-inset-*, 0px)` paddings on `.topbar`/`.composer` (`_content.scss`), `.app` left/right (`_layout.scss`), and the fixed drawer `.rail`/`.sidebar` (`_mobile.scss`). `env()` is 0 on non-notched devices, so it's a no-op there — but DON'T drop `viewport-fit=cover` or the insets all collapse to 0.
