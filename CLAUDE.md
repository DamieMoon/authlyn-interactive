...
Pi-specific machine state (SSH alias `pi` → 192.168.0.153, hostname FENRIR, port map, service layout) lives in ctx — `ctx query "authlyn pi deployment machine state"`. Re-derived via SSH 2026-05-23 after the memory wipe; the only gap is the **TP-Link DDNS hostname** (read `/etc/caddy/Caddyfile` or ask Damien).
...
2. Record the port chosen and what it's for in ctx (the authlyn pi-deployment block).
...
- Pi-side machine state (chosen ports, install layout) lives in ctx (`ctx query "authlyn pi deployment"`).
