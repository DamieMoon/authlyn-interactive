...
- Deck only: `https://authlyndev.damienmoon.sh` (cloudflared, publicly-trusted cert — the iOS/WebKit review + probe target), served by the `authlyn-test` systemd service, ns `authlyn` / db `test`. The LAN-IP `192.168.0.239:3434` (self-signed) is retired. NEVER touch prod, `main`, or `SURREAL_DB=prod`.
...
2. Push the branch to BOTH remotes (they serve different purposes — push both):
   ...
   - **Deck source** — novahome `~/authlyn-testdeck`'s `origin` is the LOCAL bare `/home/damien/authlyn-testdeck.git`, NOT GitHub (and novahome CANNOT auth to the private GitHub repo). Push there or step 3 builds STALE code: `git push novahome <branch>` (the `novahome` git remote is repointed to the `ssh novahome` alias → the bare repo; the raw-IP `ssh://damien@192.168.0.239/...` URL fails publickey because the key binding is scoped to `Host novahome`). (The step-3 SHA-verify is what catches a stale bare repo — do not skip it.)
3. Build on novahome (`ssh novahome`):
...
5. Verify from the Mac: `curl -sk -o /dev/null -w '%{http_code}\n' -m 10 https://authlyndev.damienmoon.sh/` (expect a normal response, not a connection failure), and confirm the deployed SHA matches `<sha>` (the `deploy-test.sh` output prints `DEPLOY OK (<sha>)`).
