...
// offline fallback. Default fetch cache mode is correct here: it honors the
// server's declared policy — thumbnails are NOT immutable per URL (their
// bytes change when the thumbnail pipeline version bumps), so the server
// serves them with a revalidating max-age + a pipeline-version ETag (review
// M-25), and the default mode lets the HTTP cache do that conditional
// revalidation; a pipeline bump reaches clients via the changed ETag.
...
self.addEventListener("fetch", (event) => {
  ...
  // Media blobs are session-gated server-side, so Cache Storage persistence
  // is restricted: thumbnails (`?w=N`, small) ride the bounded,
  // logout-clearable side cache; full originals (no `w` — the lightbox,
  // multi-MB) are never persisted here and pass straight through to the
  // browser, whose HTTP cache handles them under the server's Cache-Control.
  // Mutability differs per arm (review M-25): ORIGINALS are truly immutable
  // per URL (server-minted id → fixed bytes, served `immutable`); THUMBNAILS
  // are not — their bytes change across pipeline versions, so the server
  // serves them with a revalidating max-age + pipeline-version ETag and the
  // network-first handler below naturally picks up the regenerated bytes.
